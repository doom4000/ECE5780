...
int main(void)
{
  ...
  // Set the alternate function to USART3 (AF4 for STM32F072, adjust as necessary)
  GPIOC->AFR[0] |= (4 << (1 * 4)) | (1 << (5 * 4)); // Adjust the bit positions and AF number based on datasheet
    
  // Configure USART3: baud rate, word length, stop bits, parity, etc.
  USART3->BRR = HAL_RCC_GetHCLKFreq() / 9600; // Assuming 8 MHz clock, baud rate of 9600
  ...
  USART3->CR1 |= USART_CR1_UE; // Enable USART3
   ...
   while (1) {
        ...
        if (USART3->ISR & USART_ISR_RXNE) {
            ...
            switch (receivedData) {
                case 'r': // Assuming 'r' to toggle red LED (PC6)
                    GPIOC->ODR ^= (1 << 6);
                    break;
                case 'g': // Assuming 'g' to toggle green LED (PC9)
                    GPIOC->ODR ^= (1 << 9);
                    break;
                case 'b': // Assuming 'b' to toggle blue LED (PC7)
                    GPIOC->ODR ^= (1 << 7);
                    break;
                // Add more cases as needed
                default:
                    GPIOC->ODR |= (1 << 8); // Turn on orange LED (PC8) to indicate error
                    HAL_Delay(200); // Short delay to make the error indication visible
                    GPIOC->ODR &= ~(1 << 8); // Turn off orange LED (PC8)
                    break;
            }
        }
    }
