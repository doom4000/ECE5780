...
int main(void)
{
    ...
    // Enable GPIOC clock
		...
		// Set green LED (PC9) high, ensure orange (PC8) is configured but initially off
		...
		GPIOA->PUPDR &= ~(3UL << (0 * 2)); // Clear pull-up/pull-down
		...
		// Set the Systick prority to two
		...
		// Set EXTI line 0 interrupt priority
		NVIC_SetPriority(EXTI0_1_IRQn, 1);


		// Configure SYSCFG to route PA0 to EXTI line 0
		// For PA0, this step can be skipped as the default state already maps PA0 to EXTI0
		// SYSCFG->EXTICR[0] &= ~SYSCFG_EXTICR1_EXTI0; // Not needed for PA0, shown for completeness


    // Main loop
    while (1) {
        GPIOC->ODR ^= (1 << 6); // Toggle red LED (PC6)
        HAL_Delay(500); // Delay approximately 500ms
    }
}
