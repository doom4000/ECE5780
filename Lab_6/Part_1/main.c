...
int main(void)
{
  /* USER CODE BEGIN 1 */
		...
		RCC->APB2ENR |= RCC_APB2ENR_ADC1EN; // Enable ADC1 clock, adjust for your specific MCU family
		RCC->AHBENR |= RCC_AHBENR_GPIOAEN; // Assuming potentiometer connected to GPIOA

		// Configure PA0 (or your ADC input pin) to analog mode
		...
		ADC1->CFGR1 &= ~ADC_CFGR1_EXTEN; // Disable hardware trigger by clearing EXTEN bits

		ADC1->CFGR1 |= ADC_CFGR1_CONT; // Set ADC to continuous conversion mode
		ADC1->CHSELR = ADC_CHSELR_CHSEL0; // Select channel, adjust for your input
		...
		// Initialize LED pins as outputs (PC6, PC7, PC8, PC9)
		// Output mode
		...
		// Assuming LEDs are connected to GPIOC as per your USART example
		...
		while (1) {
				...
				if (ADC1->ISR & ADC_ISR_EOC) { // Check if conversion is complete
						...
						// Example for one threshold, expand as needed for your LED setup and thresholds
				}
